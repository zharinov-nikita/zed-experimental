...
- Post-processing через External Agent (ADR 0004, `crates/agent_ui/src/dictation_post_processing.rs`): при
  ...
  только опции из `options`, которые агент объявил (`session_policy`), режим — самый строгий из объявленных: тот, что
  спрашивает разрешение на каждый инструмент (`default`), потому что каждый такой запрос приходит в Zed и отклоняется;
  `plan` читал бы файлы сам, не спрашивая. Одна сессия на Dictation Session: Resume переписывается в ней же, при
  закрытии окна она удаляется у агента (`session/delete`); если агент удаление не поддерживает, она с момента создания
  помечена скрытой (в памяти на этот запуск Zed) и не предлагается к импорту тредов.
